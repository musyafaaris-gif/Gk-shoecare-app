...
class SplashPage extends StatelessWidget {
  ...
  Future<void> mulai(BuildContext context) async {
    ...
    if (nama == null || nama.isEmpty) {
      ...
      Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const PilihChannelPage()));
    }
  }
...
class PesananContext {
  static String channel = ''; // 'locker' | 'shop' | 'online'
  static String lokasiId = '';
  static String lokasiNama = '';
  ...
  static String get labelChannel {
    if (channel == 'locker') return 'GK Clean Locker - $lokasiNama';
    if (channel == 'online') return 'GK Online';
    return 'GK Shop';
  }
}

class PilihChannelPage extends StatelessWidget {
  const PilihChannelPage({super.key});

  void pilih(BuildContext context, String channel) {
    PesananContext.reset();
    PesananContext.channel = channel;
    if (channel == 'locker') {
      Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const PilihLokasiPage()));
    } else {
          ...
          child: Column(
            children: [
              const Padding(
                ...
                child: Text('Pilih cara serah sepatu kamu', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
              ),
              Card(
                child: ListTile(
                  leading: const Icon(Icons.inbox),
                  title: const Text('GK Clean Locker', style: TextStyle(fontWeight: FontWeight.bold)),
                  subtitle: const Text('Taruh sepatu di loker partner kami (cafe, lapangan padel, gym, dll)'),
                  onTap: () => pilih(context, 'locker'),
                ),
              ),
              ...
              Card(
                child: ListTile(
                  ...
                  title: const Text('GK Shop', style: TextStyle(fontWeight: FontWeight.bold)),
                  subtitle: const Text('Datang & serahkan sepatu langsung ke toko kami'),
                  onTap: () => pilih(context, 'shop'),
                ),
              ),
              const SizedBox(height: 12),
              Card(
                child: ListTile(
                  ...
                  title: const Text('GK Online', style: TextStyle(fontWeight: FontWeight.bold)),
                  subtitle: const Text('Dijemput & diantar ke lokasi kamu, ada ongkir sesuai jarak'),
                  onTap: () => pilih(context, 'online'),
                ),
              ),
            ],
          ),
...
class _PilihLokasiPageState extends State<PilihLokasiPage> {
  ...
  Future<void> muat() async {
    ...
    try {
      ...
      final hasil = docs.map((doc) {
        ...
        return {
          'id': name.split('/').last,
          'nama': fields['nama']?['stringValue'] as String? ?? '',
          'alamat': fields['alamat']?['stringValue'] as String? ?? '',
        };
      }).where((l) => l['nama']!.isNotEmpty).toList();
      ...
      setState(() {
        error = 'Gagal memuat lokasi: $e';
        sedangMemuat = false;
      });
    }
  }

  void pilihLokasi(Map<String, String> l) {
    PesananContext.lokasiId = l['id']!;
    PesananContext.lokasiNama = l['nama']!;
    Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const PilihJenisBarangPage()));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFFF5B315),
      appBar: AppBar(
        title: const Text('Pilih Lokasi Loker'),
        backgroundColor: Colors.black,
        foregroundColor: Colors.white,
      ),
      body: SafeArea(
        child: sedangMemuat
            ...
            : error != null
                ...
                : daftar.isEmpty
                    ? const Center(child: Padding(padding: EdgeInsets.all(24), child: Text('Belum ada lokasi loker terdaftar. Hubungi admin.')))
                    : ListView.builder(
                        padding: const EdgeInsets.all(16),
...
class _PilihJenisBarangPageState extends State<PilihJenisBarangPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFFF5B315),
      appBar: AppBar(
        title: Text('GK. SHOECARE', style: const TextStyle(fontSize: 16)),
        backgroundColor: Colors.black,
        foregroundColor: Colors.white,
        bottom: PreferredSize(
          ...
          child: Padding(
            ...
            child: Text(PesananContext.labelChannel, style: const TextStyle(fontSize: 11, color: Colors.white70)),
          ),
        ),
        actions: [
          IconButton(
            icon: const Icon(Icons.swap_horiz),
            tooltip: 'Ganti Channel',
            onPressed: () {
              Navigator.pushAndRemoveUntil(
                context,
                MaterialPageRoute(builder: (context) => const PilihChannelPage()),
                (route) => false,
              );
            },
          ),
...
class _LokerPageState extends State<LokerPage> {
  ...
  Future<void> muatLoker() async {
    ...
    try {
      ...
      final data = jsonDecode(response.body);
      final docs = (data['documents'] as List?) ?? [];
      final prefix = '${PesananContext.lokasiId}__';
      final nomorList = <String>[];
      ...
      for (final doc in docs) {
        ...
        final fields = doc['fields'] as Map<String, dynamic>? ?? {};
        final nomor = fields['nomor']?['stringValue'] ?? docId.substring(prefix.length);
        nomorList.add(nomor);
        hasil[nomor] = fields['terisi']?['booleanValue'] ?? false;
      }
      nomorList.sort((a, b) => (int.tryParse(a) ?? 0).compareTo(int.tryParse(b) ?? 0));
      setState(() {
        daftarNomor = nomorList;
        statusLoker = hasil;
        sedangMemuat = false;
      });
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFFF5B315),
      appBar: AppBar(
        title: Text('Loker - ${PesananContext.lokasiNama}'),
        backgroundColor: Colors.black,
        foregroundColor: Colors.white,
                ...
                : daftarNomor.isEmpty
                    ? const Center(
                        child: Padding(
                          ...
                          child: Text('Belum ada loker terdaftar di lokasi ini. Hubungi admin.'),
                        ),
                      )
                    : Padding(
                        padding: const EdgeInsets.all(16),
                        child: GridView.builder(
                          ...
                          itemCount: daftarNomor.length,
                          itemBuilder: (context, index) {
                            final nomor = daftarNomor[index];
                                  ...
                                  : () {
                                      Navigator.push(
                                        ...
                                        MaterialPageRoute(
                                          builder: (context) => PaymentPage(
                                            lokerNomor: nomor,
                                            metodePesan: PesananContext.labelChannel,
                                            ongkir: 0,
                                          ),
                                        ),
                                      );
...
class _PaymentPageState extends State<PaymentPage> {
    ...
    final response = await http.post(
      ...
      body: jsonEncode({
        'fields': {
          ...
          'lokasiId': {'stringValue': PesananContext.channel == 'locker' ? PesananContext.lokasiId : ''},
          'lokasiNama': {'stringValue': PesananContext.channel == 'locker' ? PesananContext.lokasiNama : ''},
  ...
  Future<void> kunciLoker(String namaCustomer) async {
    if (PesananContext.channel != 'locker') return;
    ...
    final lokerId = '${PesananContext.lokasiId}__$lokerTerpilih';
    final cekUri = Uri.parse('$firestoreBase/lokers/$lokerId');
  ...
  Future<void> kirimSemuaPesanan() async {
    ...
    try {
      ...
      if (PesananContext.channel == 'locker') buffer.writeln('Nomor Loker: $lokerTerpilih');
